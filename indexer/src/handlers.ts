...
indexer.onEvent({contract:game,event:'Snapshot'},async({event,context})=>{
  const p=event.params,id=`${version}:${p.matchId}`;
  const match=await context.Match.get(id);
  if(match){context.Match.set({...match,...snapshotScores(p.state),played:true,status:match.status===1?2:match.status,replayAvailability:match.status===1?"recording":match.replayAvailability});}
  if(match?.replayAvailability!=="pruned")context.Frame.set({id:`${id}:${p.version}`,matchId:id,version:p.version,state:p.state,clock:p.clock,nextAt:p.nextAt,nextKind:Number(p.nextKind),block:BigInt(event.block.number)});
...
indexer.onEvent({contract:game,event:'MatchEnded'},async({event,context})=>{
  ...
  await retainFinished(context,{...match,status:Number(p.status),winner:p.winner.toLowerCase()},`${String(event.block.number).padStart(20,"0")}:${String(event.logIndex).padStart(10,"0")}`);
...
for(const [version,game] of [["v2","GameV2"],["v3","GameV3"],["v4","GameV4"]] as const) indexer.onEvent({contract:game,event:"HandicapSet"},async({event,context})=>{
 ...
 const match=await context.Match.get(`${version}:${p.matchId}`);if(match?.replayAvailability==="pruned")return;
