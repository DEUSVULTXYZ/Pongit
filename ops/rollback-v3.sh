#!/usr/bin/env bash
set -euo pipefail
test "$#" -eq 1 && [[ "$1" =~ ^[a-f0-9]{40}$ ]] || { echo 'Usage: rollback-v3.sh COMMIT'; exit 1; }
target="/opt/pongit/releases/$1"
test -f "$target/deployments/testnet.json"
cd /opt/pongit/current
# Refuse a V1 binary or a different set of immutable contracts. Signed V3 jobs
# cannot safely be handed to the V1/V2 relayers.
docker compose exec -T relayer node -e '
 ...
 if(target.version!==3 || ["chainId","game","market","vault","tournaments","arcade"].some(k=>target[k]!==active[k]) || JSON.stringify(target.legacy)!==JSON.stringify(active.legacy)) process.exit(1);
...
echo "V3 application restored to $1; contract deployments and persistent journal preserved."
