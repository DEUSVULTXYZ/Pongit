...
contract MockResultsV2 is IMatchResult {
...
contract BuyerHandlerV2 is Test {
...
contract MarketV2InvariantTest is StdInvariant, Test {
    Market market;
    Vault vault;
    BuyerHandlerV2 handler;

    function setUp() public {
        vault = new Vault(address(this));
        market = new Market(address(this), payable(address(this)), new MockResultsV2(), new LMSR(), vault);
        vault.registerModule(address(market));
        vault.registerModule(address(this));
        vault.seal();
        handler = new BuyerHandlerV2(market);
