...
contract Game is AccessControl, Pausable, EIP712, IMatchResult, IGameClock {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bytes32 public constant JOIN_TYPEHASH = keccak256(
        "Join(address player,address opponent,bytes32 roomId,bytes32 commitment,address sessionKey,uint256 nonce,uint64 deadline,uint64 sessionExpiry,uint32 maxInputs,uint256 tournamentId)"
    );
    ...
    mapping(address => uint256) public lastSeason;
    ...
    constructor(address admin) EIP712("PONG", "1") {
    ...
    function clock(uint256 id) public view returns (uint64) {
        ...
        return m.status < 2 ? 0 : (activeBlock() - m.startBlock) * BLOCK_US;
    }
        ...
        returns (uint256 id)
    {
        ...
        emit MatchCreated(id, a.player, b.player, a.tournamentId);
    }
    ...
    function reveal(uint256 id, address player, bytes32 secret) external whenNotPaused {
        ...
        if (m.revealedA && m.revealedB) {
            m.state = Physics.initial(keccak256(abi.encode(m.secretA, m.secretB, id)));
            m.status = 2;
    ...
    function _advance(uint256 id, uint256 limit) private returns (bool caught) {
        Match storage m = matches[id];
        (Physics.State memory state, bool done) = Physics.advance(m.state, clock(id), limit);
        m.state = state;
        if (state.finished) {
            m.status = 3;
    ...
    function submitInput(Input calldata input, bytes calldata signature) external whenNotPaused {
        ...
        require(session.rateCount < maxInputsPerBlock, "rate limit");
    ...
    function ratingOf(address player) public view returns (Rating memory r) {
        ...
        r = ratings[season][player];
        if (r.elo != 0) return r;
        uint256 previous = lastSeason[player];
        int256 value = previous == 0 ? int256(1000) : int256(uint256(ratings[previous][player].elo));
    ...
    function _rate(Match storage m) private {
        Rating memory a = ratingOf(m.playerA);
        Rating memory b = ratingOf(m.playerB);
        int256 diff = int256(uint256(b.elo)) - int256(uint256(a.elo));
        if (diff > 2000) diff = 2000;
        if (diff < -2000) diff = -2000;
        int256 expected = 1e36 / (1e18 + SD59x18.unwrap(exp(sd(diff * 5756462732485114))));
        ...
        uint256 season = currentSeason();
        ratings[season][m.playerA] = a;
        ratings[season][m.playerB] = b;
        lastSeason[m.playerA] = season;
        lastSeason[m.playerB] = season;
        emit RatingUpdated(m.playerA, season, a.elo, a.played, a.wins);
        emit RatingUpdated(m.playerB, season, b.elo, b.played, b.wins);
    }
