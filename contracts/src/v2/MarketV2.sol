...
import {IMatchResultV2 as IMatchResult} from "./GameV2.sol";
import {Vault} from "../Vault.sol";

interface IMarketMaker {
    function cost(uint256 a, uint256 b, uint256 liquidity) external pure returns (uint256);
}

contract LMSRV2 is IMarketMaker {
    /// @dev Stable log-sum-exp: never exponentiate a positive value. Error budget and
    /// the reserve postcondition live in Market; quotes must never be used as collateral.
    function cost(uint256 a, uint256 b, uint256 liquidity) external pure returns (uint256) {
        require(liquidity >= 1e15 && liquidity <= 1e24 && a <= 1e27 && b <= 1e27, "domain");
        uint256 hi = a > b ? a : b;
        uint256 difference = a > b ? a - b : b - a;
        int256 z = -int256(difference * 1e18 / liquidity);
        if (z < -40e18) return hi;
        int256 tail = SD59x18.unwrap(ln(sd(1e18 + SD59x18.unwrap(exp(sd(z))))));
        return hi + uint256(tail) * liquidity / 1e18;
    }
}

contract MarketV2 is AccessControl, Pausable, EIP712, ReentrancyGuard {
    ...
    function claim(uint256 id, address player) external nonReentrant {
        ...
        b.totalPaid -= p.paid;
        b.reserve -= amount;
        vault.depositFor{value: amount}(player);
