...
interface IPreviousGame {
    function ratingFor(address player, uint8 mode) external view returns (uint32, uint32, uint32);
...
contract GameV3 is AccessControl, Pausable, EIP712, IMatchResultV2, IGameClockV2 {
    error GameRule();
    ...
    ArcadeSessions public immutable arcade;
    ...
    event RatingUpdated(
        address indexed player, uint256 indexed season, uint8 indexed mode, uint32 elo, uint32 played, uint32 wins
    );
    event SessionChanged(uint256 indexed matchId, address indexed player, address key);

    constructor(address admin, address previousGame_, ArcadeSessions arcade_) EIP712("PONG", "1") {
        require(admin != address(0), GameRule());
    ...
    function setMarket(address value) external {
        require(msg.sender == configurator && market == address(0) && value.code.length > 0 && nextId == 1, GameRule());
        require(IPressure(value).results() == address(this), GameRule());
        market = value;
    }
    ...
    function setInputLimit(uint8 value) external onlyRole(ADMIN_ROLE) {
        require(value > 0 && value <= 10, GameRule());
    ...
    function _checkJoin(Join calldata j, bytes calldata sig) private {
        require(j.player != address(0) && j.sessionKey != address(0) && j.commitment != bytes32(0), GameRule());
        require(
            j.deadline >= block.timestamp && j.sessionExpiry > block.timestamp
                && j.sessionExpiry <= block.timestamp + 1 days,
            GameRule()
        );
        require(j.maxInputs > 0 && j.maxInputs <= 20000 && j.nonce == nonces[j.player]++, GameRule());
        arcade.checkJoin(j.player, joinDigest(j), sig, j.sessionKey, j.sessionExpiry);
    }
    ...
    function createMatch(Join calldata a, bytes calldata sigA, Join calldata b, bytes calldata sigB)
        ...
        returns (uint256 id)
    {
        require(a.player != b.player && a.opponent == b.player && b.opponent == a.player, GameRule());
        require(a.roomId == b.roomId && a.tournamentId == b.tournamentId && !usedRooms[a.roomId], GameRule());
        require(market != address(0), GameRule());
        require(activeMatchOf[a.player] == 0 && activeMatchOf[b.player] == 0, GameRule());
        require(pendingRatingOf[a.player] == 0 && pendingRatingOf[b.player] == 0, GameRule());
        require(
            a.mode <= 1 && a.mode == b.mode && a.ranked == b.ranked && a.rulesVersion == 2 && b.rulesVersion == 2,
            GameRule()
        );
        require(a.tournamentId == 0 || a.mode == 0 && a.ranked, GameRule());
        _checkJoin(a, sigA);
        _checkJoin(b, sigB);
        usedRooms[a.roomId] = true;
        id = nextId++;
        activeMatchOf[a.player] = id;
        activeMatchOf[b.player] = id;
        Match storage m = matches[id];
        m.playerA = a.player;
        m.playerB = b.player;
        ...
        m.mode = a.mode;
        m.ranked = a.ranked;
        m.rulesVersion = a.rulesVersion;
        m.inputLimit = maxInputsPerBlock;
    ...
    function reveal(uint256 id, address player, bytes32 secret) external whenNotPaused {
        Match storage m = matches[id];
        require(m.status == 1 && block.number <= m.createdBlock + 200, GameRule());
        if (player == m.playerA) {
            require(!m.revealedA && keccak256(abi.encodePacked(secret)) == m.commitA, GameRule());
            m.secretA = secret;
            m.revealedA = true;
        } else {
            require(player == m.playerB && !m.revealedB && keccak256(abi.encodePacked(secret)) == m.commitB, GameRule());
            m.secretB = secret;
            m.revealedB = true;
        }
    ...
    function cancelUnstarted(uint256 id) external {
        ...
        require(m.status == 1 && block.number > m.createdBlock + 200, GameRule());
        m.status = 4;
        delete activeMatchOf[m.playerA];
        delete activeMatchOf[m.playerB];
    ...
    function resolveEvent(uint256 id) external whenNotPaused {
        require(matches[id].status == 2, GameRule());
        _advance(id, 64);
    }
    ...
    function _advance(uint256 id, uint256 limit) private returns (bool caught) {
        ...
        if (state.awaitingServe) {
            state.t = target;
            state.resumeAt = target + 3_000_000;
        }
        m.state = state;
        if (state.finished) {
            m.status = 3;
            ...
            if (m.ranked) {
                pendingRatingOf[m.playerA] = id;
                pendingRatingOf[m.playerB] = id;
            }
    ...
    function submitInput(Input calldata input, bytes calldata signature) external whenNotPaused {
        ...
        require(m.status == 2, GameRule());
        require(input.direction >= -1 && input.direction <= 1, GameRule());
        require(
        ...
        require(input.player == m.playerA || input.player == m.playerB, GameRule());
        require(
            session.key != address(0) && block.timestamp <= session.expiry && session.remaining > 0
                && arcade.validInput(input.player, session.key),
            GameRule()
        );
        require(input.nonce == session.nonce + 1, GameRule());
        require(
        ...
        require(session.rateCount < m.inputLimit, GameRule());
        require(_advance(input.matchId, 32), GameRule());
        require(m.status == 2, GameRule());
    ...
    function authorizeSession(
        ...
        Match storage m = matches[id];
        require(m.status == 2 && (player == m.playerA || player == m.playerB), GameRule());
        require(
        ...
        arcade.checkJoin(player, _hashTypedDataV4(hash), sig, key, expiry);
    ...
    /// @param action 1 revokes the session; 2 concedes the match. An active arcade grant may perform these gameplay-only actions.
    function playerAction(address player, uint256 id, uint8 action, uint256 nonce, uint64 deadline, bytes calldata sig)
        external
    {
        Match storage m = matches[id];
        require(m.status == 2 && (player == m.playerA || player == m.playerB), GameRule());
        require(deadline >= block.timestamp && nonce == nonces[player]++, GameRule());
        arcade.checkSignature(
            player, _hashTypedDataV4(keccak256(abi.encode(ACTION_TYPEHASH, player, id, action, nonce, deadline))), sig
        );
        if (action == 1) {
    ...
    function ratingOf(address player) public view returns (Rating memory) {
        return ratingFor(player, 0);
    }

    function ratingFor(address player, uint8 mode) public view returns (Rating memory r) {
        require(mode <= 1, GameRule());
        uint256 season = currentSeason();
        r = ratings[mode][season][player];
        if (r.elo != 0) return r;
        uint256 previous = lastSeason[mode][player];
        if (previous == 0 && previousGame != address(0)) {
            (r.elo, r.played, r.wins) = IPreviousGame(previousGame).ratingFor(player, mode);
            return r;
        }
    ...
    function finalizeRating(uint256 id) external {
        Match storage m = matches[id];
        require(m.status == 3 && !m.ratingFinalized, GameRule());
        m.ratingFinalized = true;
        delete pendingRatingOf[m.playerA];
        delete pendingRatingOf[m.playerB];
        if (!m.ranked) return;
        _rate(m);
    }
