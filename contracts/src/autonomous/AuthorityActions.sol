...
interface IAuthorityHost {
    ...
    function input(uint256 id, int8 direction, uint256 nonce, uint256 deadline) external;
    function concede(uint256 id) external;
}

interface IAuthorityFinalizer {
...
contract AuthorityActions {
    ...
    function allowedCommand(bytes4 s) external pure returns (bool) {
        return s == this.queue.selector || s == this.cancelQueue.selector || s == this.queueHeartbeat.selector
            || s == this.createRoom.selector || s == this.joinRoom.selector || s == this.leaveRoom.selector
            || s == this.rejoinQueue.selector || s == this.acceptProposal.selector || s == this.declineProposal.selector
            || s == this.inviteSomeone.selector || s == this.inviteToRoom.selector
            || s == this.answerInvitation.selector || s == this.rematch.selector || s == this.blockPlayer.selector
            || s == IAuthorityHost.input.selector || s == IAuthorityHost.concede.selector;
    }
    ...
    modifier admitting() {
        require(S.get(words, 101, S.generation(words), 0) == 0, "session draining");
        _;
    }
    ...
    function queue(uint8 mode) external engine admitting {
    ...
    function matchmake(uint8 mode, uint256 budget) external engine admitting returns (uint256) {
        return Lobby.matchmake(words, mode, budget);
    }

    function createRoom(uint8 mode) external engine admitting returns (uint256) {
        return Lobby.createRoom(words, _gameActor(), mode, false);
    }

    function joinRoom(uint256 id) external engine admitting {
    ...
    function rejoinQueue(uint256 id) external engine admitting {
    ...
    function propose(uint256 roomId) external engine admitting returns (uint256) {
    ...
    function acceptProposal(uint256 id) external engine admitting {
    ...
    function inviteToRoom(uint256 id, address recipient) external engine admitting returns (uint256) {
        return Lobby.invite(words, _gameActor(), id, recipient, 600);
    }

    function inviteSomeone(address recipient, uint8 mode) external engine admitting returns (uint256) {
    ...
    function answerInvitation(uint256 id, bool yes) external engine {
        if (yes) require(S.get(words, 101, S.generation(words), 0) == 0, "session draining");
        address actor = _gameActor();
    ...
    function rematch(uint256 source) external engine admitting returns (uint256) {
    ...
    function sessionClosure() external view returns (uint256 drainingEpoch, uint256 sealedEpoch) {
        uint256 generation = S.generation(words);
        return (S.get(words, 101, generation, 0), S.get(words, 101, generation, 1));
    }

    /// Run on the engine after the last game and outstanding proposals finish.
    /// This published marker freezes ALL application writes, so a base-chain
    /// close cannot race with a later admission or uncommitted lobby change.
    function sealCompletedSession(uint256 expectedEpoch) external engine {
        Types.Session memory d = IAuthorityHost(address(this)).hub().sessionOf(address(this), Types.GLOBAL);
        require(block.chainid == 4242 && d.status == Types.Status.Active && d.epoch == expectedEpoch, "engine epoch");
        require(
            S.get(words, 101, S.generation(words), 0) == expectedEpoch && expectedEpoch != 0, "session not draining"
        );
        require(
            IAuthorityHost(address(this)).activeCount() == 0 && Lobby.slot(words, 0) == 0 && Lobby.slot(words, 1) == 0,
            "drain matches first"
        );
        S.set(words, 101, S.generation(words), 1, expectedEpoch);
        emit SessionSealed(S.generation(words), expectedEpoch);
    }

    /// Permissionless execution, but the contract fixes what can be closed.
    /// Only the seal actually published by the hub on Monad is authoritative.
    function closeCompletedSession(uint256 expectedEpoch) external {
        require(
            block.chainid == 10143 && Control.state().execution == Control.Execution.Interlude
                && !Control.state().starting,
            "close state"
        );
        Types.Session memory d = IAuthorityHost(address(this)).hub().sessionOf(address(this), Types.GLOBAL);
        require(d.status == Types.Status.Active && d.epoch == expectedEpoch && expectedEpoch != 0, "close epoch");
        require(
            S.get(words, 101, S.generation(words), 0) == expectedEpoch
                && S.get(words, 101, S.generation(words), 1) == expectedEpoch,
            "seal not published"
        );
        require(
            IAuthorityHost(address(this)).activeCount() == 0 && Lobby.slot(words, 0) == 0 && Lobby.slot(words, 1) == 0,
        ...
        IAuthorityHost(address(this)).hub().closeDelegation(Types.GLOBAL);
        _transition(Control.Execution.Returning, keccak256("MATCH_SESSION_CLOSED"));
    }

    /// A normal renewal keeps rooms and rankings, unlike disaster recovery.
    /// No fresh delegation is opened on a still-contestable baseline.
    function renewCompletedSession(uint256 expectedEpoch) external payable {
        require(
            block.chainid == 10143 && msg.sender == IAuthorityHost(address(this)).administrator()
                && Control.state().execution == Control.Execution.Returning && !Control.state().starting
                && Control.state().reason == keccak256("MATCH_SESSION_CLOSED"),
            "renew state"
        );
        Types.Session memory d = IAuthorityHost(address(this)).hub().sessionOf(address(this), Types.GLOBAL);
        // Some hub revisions clear the released session tuple. Permissionless
        // releaseStake must not strand renewal after a valid published seal.
        require(
            expectedEpoch != 0 && (d.epoch == expectedEpoch || d.status == Types.Status.None && d.epoch == 0),
            "renew epoch"
        );
        if (d.status == Types.Status.Exiting) {
        ...
        // Lock the canonical outcomes into the non-delegated finance adapter
        // before another delegation makes its game reads contestable again.
        // This records debts/results; it does not transfer to arbitrary wallets.
        for (uint256 i = 2; i < 4; i++) {
            uint256 id = S.get(words, 101, S.generation(words), i);
            if (id == 0) continue;
            IAuthorityFinalizer(address(Control.state().pressureSource)).finalizeResult(id);
            S.set(words, 101, S.generation(words), i, 0);
        }
        S.set(words, 101, S.generation(words), 0, 0);
        S.set(words, 101, S.generation(words), 1, 0);
        _openInterlude(Control.Execution.Returning, keccak256("MATCH_SESSION_RENEWING"));
    }
    ...
    function beginRecovery() external {
        ...
        require(
            Control.state().reason != keccak256("MATCH_SESSION_CLOSED")
                || msg.sender == IAuthorityHost(address(this)).administrator(),
            "normal renewal pending"
        );
        Types.Session memory d = IAuthorityHost(address(this)).hub().sessionOf(address(this), Types.GLOBAL);
    ...
    function returnToInterlude() external payable {
        ...
        _openInterlude(Control.Execution.Monad, keccak256("ADMIN_RETURN_REQUESTED"));
    }

    function _openInterlude(Control.Execution before_, bytes32 reason) private {
        ...
        emit ExecutionChanged(
            before_, Control.Execution.Returning, S.generation(words), Control.state().reason, Control.state().changedAt
        );
    }
