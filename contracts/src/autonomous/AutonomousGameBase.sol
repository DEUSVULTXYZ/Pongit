...
import {AuthorityRating} from "./AuthorityRating.sol";
...
abstract contract AutonomousGameBase is AutonomousArenaInterludeSurface {
    ...
    function _words() internal view virtual returns (mapping(bytes32 => uint256) storage);
    ...
    /// The autonomous candidate refuses unsupported proofs; there is no attester override.
    ...
    constructor(IInterludeHub hub_, uint256 seasonGenesis) Delegatable(hub_) {
        require(seasonGenesis <= block.timestamp, "season genesis");
    ...
    function _assertExecution() internal view virtual;

    function _gameActor() internal view virtual returns (address) {
        return _actor();
    }
    function _tickUs(uint256 id) internal view virtual returns (uint256);
    function _clockLive() internal view virtual returns (bool);

    // The runtime address prevents constant-key folding, including the capacity counter.
    function _key(uint256 ns, uint256 id, uint256 field) internal view returns (bytes32) {
        return keccak256(abi.encode(address(this), ns, id, field));
    }

    function _get(uint256 id, uint256 field) internal view returns (uint256) {
        return _words()[_key(0, id, field)];
    }

    function _set(uint256 id, uint256 field, uint256 value) internal {
        _words()[_key(0, id, field)] = value;
    }
    ...
    function activeMatchOf(address player) public view returns (uint256) {
        return _words()[_key(1, uint160(player), 0)];
    }

    function activeCount() public view returns (uint256) {
        return _words()[_key(4, 0, 0)];
    }
    ...
    /// Admission is internal and can only follow two stored contract agreements.
    function _createMatch(Offer memory o) internal {
        require(o.id != 0 && o.a != address(0) && o.b != address(0) && o.a != o.b && o.mode <= 1, "match");
        if (_phase(o.id) != 0 || activeCount() >= CAPACITY || activeMatchOf(o.a) != 0 || activeMatchOf(o.b) != 0) {
            revert ArenaBusy();
        }
        uint256 meta = uint160(o.a) | (o.ranked ? 1 << 160 : 0) | (2 << 161) | (3 << 164) | (uint256(o.mode) << 168);
        _set(o.id, 0, meta);
        _set(o.id, 1, uint160(o.b));
        _set(o.id, 2, uint64(block.number) | (uint256(o.expires) << 64));
        _set(o.id, 3, uint256(keccak256(abi.encode(o.id, o.a, o.b, address(this)))));
        _set(o.id, 11, uint256(o.room));
        _words()[_key(1, uint160(o.a), 0)] = o.id;
        _words()[_key(1, uint160(o.b), 0)] = o.id;
        _words()[_key(4, 0, 0)]++;
        _save(o.id, physicsRules.initial(bytes32(_get(o.id, 3)), o.mode));
        emit MatchAccepted(o.id, o.room, o.a, o.a, o.b, o.mode, o.ranked);
        emit MatchAccepted(o.id, o.room, o.b, o.a, o.b, o.mode, o.ranked);
        _publish(o.id);
    }
    ...
    function _advance(uint256 id, bool mayResume) private returns (bool complete) {
        ...
        if (block.number >= start) target += (block.number - start) * _tickUs(id);
        if (block.number < start || target < s.t) {
            ...
            times = (times & ((uint256(type(uint128).max)) << 64)) | uint64(block.number) | (uint256(s.t) << 192);
            _set(id, 2, times);
            target = s.t;
        }
        ...
        )
    {
        ...
        uint256 clock = s.t;
        if (phase == 2 && _clockLive() && block.number >= uint64(t)) {
            uint256 elapsed = uint64(t >> 192) + (block.number - uint64(t)) * _tickUs(id);
