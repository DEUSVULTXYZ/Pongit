...
/// carry this version; historical contracts keep their original rules.
...
contract PongChaosEvents is Rooms {
    ...
    function submitRandomness(uint256 id,uint256 expectedRequest,bytes calldata signature) external engine whenNotDelegated(Types.GLOBAL){
        ...
        // Catch-up may stop at the gas reserve. Never store a revealed draw at
        // that partial clock: otherwise its submitter's gas chooses activation.
        if(!_advance(id,false)){_publish(id);return;}
