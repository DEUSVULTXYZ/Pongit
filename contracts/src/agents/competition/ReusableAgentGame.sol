...
library ReusableAgentGame {
    uint256 internal constant RULES=15;
    uint256 private constant SLOT=1;
    uint256 private constant MAX_US=360_000_000;
    struct Result {T.Result match_; uint256 brainA; uint256 brainB; uint64 finishedAt; uint256 rules;}
    error InvalidMatch();
    ...
    function cancelAdmission(mapping(bytes32=>uint256) storage w,Admission.Ticket calldata ticket,A.Binding calldata binding,
        bytes calldata signature,address signer,address authority,HousePolicies policies,RoomsRules classic,ChaosEngine kernel) external {
        Binding.cancelExpired(w,ticket,binding,signature,signer,authority,policies);
    ...
    function result(mapping(bytes32=>uint256) storage w,ChaosEngine kernel) public view returns(Result memory r){
        ...
        r.match_=T.Result(T.Ref(10143,address(this),S.get(w,31),S.get(w,37)),a,b,winner==1?a:winner==2?b:address(0),
            bytes32(S.get(w,9)),uint8(m>>168&1),phase(w),p.scoreA,p.scoreB,p.t,false);
        r.brainA=S.get(w,51);r.brainB=S.get(w,52);r.finishedAt=uint64(S.get(w,19));r.rules=RULES;
    }
    function finish(mapping(bytes32=>uint256) storage w,ChaosEngine kernel,uint8 status,address winner) public {
        ...
        require(status==4?winner==address(0):winner==a||winner==b||winner==address(0),"winner");
        ...
        S.set(w,9,uint256(keccak256(abi.encode(address(this),RULES,S.get(w,31),S.get(w,37),a,b,winner,
            status,uint8(m>>168&1),m>>160&1==1,p.scoreA,p.scoreB,p.t,S.get(w,19),S.get(w,36),S.get(w,51),S.get(w,52)))));
    ...
    function advance(mapping(bytes32=>uint256) storage w,RoomsRules classic,ChaosEngine kernel,HousePolicies policies,IInterludeHub hub,uint256 target) private returns(bool){
        A.Binding memory binding=Binding.binding(w);uint64 regulation=300_000_000;
        uint64 limit=binding.overtime?uint64(MAX_US):regulation;if(target>limit)target=limit;
        // Steer fixed physical storage while all externally visible identities
        // (randomness, snapshots, authorization, results) stay logical.
        binding.id=SLOT;
        for(uint8 pass;pass<16;pass++){
            ...
            end=PoolSteer.steer(w,binding,policies,kernel,end);p=state(w,kernel);
            if(p.mode==0){
    ...
    function input(mapping(bytes32=>uint256) storage w,RoomsRules classic,ChaosEngine kernel,HousePolicies policies,IInterludeHub hub,
    ...
    function tick(mapping(bytes32=>uint256) storage w,RoomsRules classic,ChaosEngine kernel,HousePolicies policies,IInterludeHub hub) external {
        if(phase(w)!=2)revert InvalidMatch();advance(w,classic,kernel,policies,hub,clockTarget(w,kernel));publish(w,kernel);
    }
    function concede(mapping(bytes32=>uint256) storage w,RoomsRules classic,ChaosEngine kernel,HousePolicies policies,IInterludeHub hub,uint8 side) external {
        ...
        record(w,side,4,0,uint64(target>MAX_US?MAX_US:target));advance(w,classic,kernel,policies,hub,target);publish(w,kernel);
    }
    function randomness(mapping(bytes32=>uint256) storage w,RoomsRules classic,ChaosEngine kernel,HousePolicies policies,IInterludeHub hub,uint256 expected,bytes calldata signature) external {
        ...
        if(advance(w,classic,kernel,policies,hub,clockTarget(w,kernel))&&phase(w)==2){
