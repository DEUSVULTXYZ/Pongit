...
export type RpcMetric={at:number;target:"interlude"|"monad"|"pongit";method:string;status:number;ms:number;source:"network"|"cooldown"|"cache"|"websocket";requestId?:string;requestBytes?:number};
...
export function measuredFetch(target:RpcMetric["target"]):typeof fetch {
 return async(input,init)=>{
  ...
  const requestBytes=typeof init?.body==='string'?new TextEncoder().encode(init.body).byteLength:undefined;
  ...
  try{const response=await fetch(input,init);recordRpc({at,target,method,status:response.status,ms:Date.now()-at,source:"network",requestBytes,requestId:(response.headers.get("fly-request-id")||response.headers.get("x-request-id")||"").slice(0,160)||undefined});return response;}
