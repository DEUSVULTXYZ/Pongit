...
export function publicIndependentManifest(raw:any):IndependentManifest{
 if(raw?.rulesVersion!==undefined&&![4,12,13].includes(raw.rulesVersion))throw Error('Unsupported independent rules');
 ...
 return {...addresses,chainId:10143,rulesVersion:raw.rulesVersion??4,arenas,genesis:Number(raw.genesis),createdAt:String(raw.createdAt),...(raw.startBlock!==undefined?{startBlock:String(raw.startBlock)}:{})} as IndependentManifest;
}
