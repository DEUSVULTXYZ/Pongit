...
const clamp = (y: bigint) =>
  y < HALF_PADDLE
    ? HALF_PADDLE
    : y > HEIGHT - HALF_PADDLE
      ? HEIGHT - HALF_PADDLE
      : y;
...
export function initial(seed: Hex): State {
  return serve({
    x: 0n,
    y: 0n,
    vx: 0n,
...
export function move(s: State, to: bigint): State {
  ...
  return {
    ...
    x: s.x + (s.vx * dt) / SCALE,
    y: s.y + (s.vy * dt) / SCALE,
    left: clamp(s.left + (BigInt(s.leftDir) * PADDLE_SPEED * dt) / SCALE),
    right: clamp(s.right + (BigInt(s.rightDir) * PADDLE_SPEED * dt) / SCALE),
    t: to,
  };
}
