...
export async function readPoolPermission(node:PublicClient,app:Address,id:bigint,side:0|1,initial:{key:Address;expires:bigint},reusable=false){
 const at=BigInt(54+side*3),head=await node.getBlockNumber();
 const values=await Promise.all([at,at+1n,at+2n].map(async field=>{
  const key=keccak256(encodeAbiParameters([{type:'address'},{type:'uint256'},{type:'uint256'},{type:'uint256'}],[app,0n,reusable?1n:id,field]));
