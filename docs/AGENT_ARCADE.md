...
- The current 24-hour soak started on September 14 at 00:54:51 UTC and is scheduled to end on September 15 at 00:54:51 UTC. Earlier trials were stopped during fixes and retained as incomplete reports, including the cached-grant renewal issue described below and adding the external controller's traffic measurements. Source hashes, sample gaps, availability and storage deltas are recorded; elapsed time alone does not open the service.
...
- Real shared-indexer backfill passed again at 01:04 UTC: 30 agent results matched published identities, modes and scores alongside eight human results; 20 shared player lists had the correct latest-three ordering and 19 old results retained summaries after replay pruning. The private rebuild starts at the unchanged production history boundary, block 62260200; it does not reset the human database.
- A live exported PostgreSQL snapshot restored successfully with exact counts for all 16 agent tables, without stopping games. Its private key/grant backup was copied off the VPS and its dump checksum verified. Production backup scripts now include the optional agent database, metadata and bot command journal; their final configured restore check remains part of rollout.
- Required before activation: the complete 24-hour observation and review of its actual availability, renewals, traffic, cost and effect coverage, followed by final public smoke tests.
