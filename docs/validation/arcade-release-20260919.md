...
## Human renewal and new-epoch games, 20 September 00:35 UTC

The rules-9 human rehearsal released epoch 1 normally after 659 batches. Release transaction `0x23a5bb4879e46342b1534d648216e94d3ed15ee3848919f901127953526a8b0a`, block 64023205, used 8,208,115 gas. The measured wait from closure was 3,601 seconds. Three financial results were finalized, then renewal `0xfbad44dd7e14327c2f6c916919614faaf1cb928c819804072909461212480028`, block 64023302, opened epoch 2. The hosted epoch was verified available at 00:20:20 UTC. All operations used the existing operator journal; public production stayed closed and unchanged.

The first new-epoch live trial completed Classic but the harness aborted Chaos when drand temporarily failed to supply its committed round. Its report remains failed and preserved. The harness incorrectly routed beacon transport errors through its financial-failure flag; it now records a retry for `BEACON_UNAVAILABLE` or `DRAND_RATE_LIMIT` and keeps playing. Actual submission or nonce-reconciliation errors still fail the strict harness. No randomness substitute was introduced.

A separate scoped recovery used the saved fixture grant, checked that no command was uncertain, observed/published the terminal 7–6 result and completed its 0.006 test-MON payout. It is recovery evidence, not a replacement passing natural-game report. Its initial financial step lacked the pressure signer file setting; that setup failure is preserved separately. The second attempt completed without another game or altered winner.

Fresh run 5 then passed both modes in epoch 2: Classic 5–7 with 102 confirmed changes per player; Chaos 6–7 with 125 and 102 changes, four verified randomness proofs, realtime betting pressure and a 0.006 test-MON payout to the synthetic beneficiary without a recipient signature. Four compact-session reconstructions reused their grants. This is not physical passkey evidence. Command-to-confirmed-read medians/p95 were 121/149 ms over 204 Classic inputs and 123/144 ms over 227 Chaos inputs. These are this scenario's measurements, not a before/after claim for all traffic or a worst-case bound.

The corrected agent pool also passed real private HTTP catalogue/history/reference checks and actual Monad archive-event verification; see [the series report](../AGENT_SERIES.md). A new isolated Envio database indexed six human browser results at rules 9 and both agent results at rules 11, alongside ten historical rules-7 agent results. All 19 observed player lists had the correct shared latest-three ordering; two pruned summaries remained. The direct live-physics harness does not emit `recordMatch`, so its results were not falsely included in the expected indexed count. This verifies result indexing, not the still-unqualified pool replay-frame recording/delivery.

No final 24-hour trial or public reopening is implied. Full tournaments, sufficient reserve arenas, maximum-load release bounds, new-series human challenges, frame/replay delivery and the final unchanged trial remain gates.

After these checks, `epoch2-and-series3-20260920T0036Z` backed up the human fixture, series operations, shared private indexer, candidate secrets and shared operator journal. All five payload checksums matched their off-VPS copies. The public release remains `7d35926`; the disk is 74% occupied. The separate earlier two-file backup precedes the idle private controller update, so that update also has its previous runtime configuration and database preserved.
