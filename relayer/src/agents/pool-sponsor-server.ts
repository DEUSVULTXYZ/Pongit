...
const releaseEvidence=agentPoolReleaseEvidence(manifest);
assert([3,4].includes(manifest.version) && releaseEvidence && BigInt(releaseEvidence) !== 0n);
assert.equal(releaseEvidence.toLowerCase(), process.env.PONG_AGENT_POOL_RELEASE_EVIDENCE?.toLowerCase());
