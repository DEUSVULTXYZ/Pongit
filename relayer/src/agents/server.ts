...
export async function startAgentService(options:{db:Pool;manifest:AgentManifest;key:Hex;rpcUrl:string;port:number;host?:string;origin:string;houseAddresses?:string[];trustProxy?:boolean;
 ...
 async function catalog():Promise<AgentProfile[]>{
  const rows=(await db.query(`SELECT i.*,p.available,p.seen,o.match_id FROM agent_arcade.identities i LEFT JOIN agent_arcade.presence p ON p.app=i.app AND p.player=i.agent
   LEFT JOIN agent_arcade.occupancy o ON o.app=i.app AND o.player=i.agent WHERE i.app=$1 ORDER BY i.kind DESC,i.name,i.agent LIMIT 500`,[app])).rows;
  return rows.map(r=>({agent:r.agent,creator:r.creator,name:r.name,avatar:r.avatar,kind:r.kind,modes:([0,1] as const).filter(mode=>r.modes&(1<<mode)),
   qualification:r.qualification,available:!!r.available&&Date.now()-new Date(r.seen).getTime()<30000,...(r.match_id?{playing:ref(r.match_id)}:{}),createdAt:r.created_at}));
 }
 async function ownMatch(player:string){return(await db.query(`SELECT m.* FROM agent_arcade.occupancy o JOIN agent_arcade.matches m ON m.id=o.match_id WHERE o.app=$1 AND o.player=$2`,[app,player])).rows[0];}
 const server=createServer(async(req,res)=>{
  ...
  try{
   ...
   if(req.method==='GET'&&path==='/config'){send(res,{...m,health:coordinator.health()});return;}
   ...
   if(req.method==='POST'&&path==='/register'){
    ...
    rate(`register:${ip}`,6);
    const r=z.object({creator:address,agent:address,name:z.string(),avatar:z.number().int(),modes:z.number().int().min(1).max(3),expires:z.coerce.bigint(),creatorProof:proof,agentProof:proof}).parse(await body(req));
    ...
    const [creator,agent]=await Promise.all([r.creatorProof,r.agentProof].map(signature=>recoverTypedDataAddress({domain,types:agentRegistrationTypes,primaryType:'AgentRegistration',message:registration,signature:signature as Hex})));
    if(creator.toLowerCase()!==r.creator||agent.toLowerCase()!==r.agent||r.agent===r.creator)throw Error('Both owners must sign the agent registration');
    const current=await coordinator.client.read('agentIdentity',[r.agent]) as readonly [Address,number,number,Hex];
    if(BigInt(current[0])===0n){await coordinator.writer.send(`register:${r.agent}:${r.expires}`,'registerAgent',[registration,r.creatorProof,r.agentProof]);}
    else if(current[0].toLowerCase()!==r.creator||current[1]!==r.modes||current[3]!==metadata)throw Error('This agent identity is already registered');
    const qualification=Object.fromEntries([0,1].filter(mode=>r.modes&(1<<mode)).map(mode=>[mode,'registered']));
    await db.query(`INSERT INTO agent_arcade.identities(app,agent,creator,name,avatar,kind,modes,qualification) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(app,agent) DO NOTHING`,
     [app,r.agent,r.creator,r.name,r.avatar,house.has(r.agent)?'pongit':'community',r.modes,qualification]);send(res,{agent:r.agent,qualification});return;
   }
   ...
   if(req.method==='GET'&&path==='/me'){
    ...
    send(res,{player,match:match?{...match,ref:ref(match.id),accepted}:null,
