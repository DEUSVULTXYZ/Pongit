...
import type {ReusableResultCandidate} from '../../shared/reusable-results';
...
function validate(c:ReusableResultCandidate){
 if(![14,15].includes(c.rules)||!Number.isInteger(c.index)||c.index<0||c.index>=RESULT_TREE_CAPACITY
  ||!/^0x[0-9a-f]{64}$/i.test(c.root)||!/^0x[0-9a-f]{64}$/i.test(c.transactionHash)
...
export function createReusableResultArchive(db:Pool){
 ...
 async function proof(ref:ResultEpoch,published:PublishedCommitment,matchId:bigint,selectedLeaves?:readonly Hex[]){
  ...
  const rows=(await db.query(`SELECT DISTINCT position,leaf,root,match_id,rules,ticket_hash,result_hash,canonical
   FROM il_reusable_results WHERE chain_id=$1 AND app=$2 AND epoch=$3 AND position<$4 ORDER BY position LIMIT 131073`,
