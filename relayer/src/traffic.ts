...
export function trafficBudget(method:string|undefined,path:string) {
 if(method==='GET'&&path==='/independent/config')return {bucket:'arena-health',limit:6000};
 if(method==='GET'&&/^\/independent\/operations\/0x[\da-f]{64}$/i.test(path))return {bucket:'receipt',limit:2400};
 ...
 if(method==="POST" && path==="/region-probe")return {bucket:"region-probe",limit:30};
