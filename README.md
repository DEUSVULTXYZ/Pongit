...
**12 September testnet status:** Classic and Chaos run on the compatible recovery release. The expired shared delegation was recovered, and real paid Chaos games published and settled on Monad. The [independent-arena candidate](docs/INDEPENDENT_ARENAS.md) now has a contract-owned lobby and passed a real three-session isolation rehearsal. Complete candidate games still encounter a hosted [batch-2 publication failure](docs/INTERLUDE_PUBLICATION_413.md), so the public home has not migrated. V4 balances, markets and tournaments remain available separately.
...
- **Two-hour arcade grants** survive F5 in the same tab. Previously issued shorter grants retain their signed expiry. Remembered passkeys keep gameplay flowing; finance still asks for the wallet passkey.
...
Prediction does not make an unconfirmed input authoritative, and betting locks reduce latency advantage without eliminating it. [Rooms](docs/CHAOS_ROOMS.md) use a hosted engine and scoped Mera sessions; financial operations and the preserved V4 arcade execute on Monad. Chaos intermissions include a 40-block betting window. New testnet payouts use the first published result without waiting for challenge finality; corrections are logged without duplicate payment. The shared deployment can still require a drain for renewal. The [recovery report](docs/RELIABLE_SESSIONS.md) distinguishes deployed behavior from the [independent-arena candidate](docs/INDEPENDENT_ARENAS.md). Physical cross-device passkey recovery still needs a demonstration. This is a testnet release without an external security audit or a claim of bug-free operation.

See [Night Shift UX](docs/NIGHT_SHIFT.md) for the cabinet navigation, recent-match API and audio credits.

The older [contract-authority design](docs/CONTRACT_AUTHORITY.md) included a shared delegation and Monad gameplay fallback. It is superseded by the [independent-arena design and qualification record](docs/INDEPENDENT_ARENAS.md): common business contracts on Monad, one physics delegation per arena, and capacity waiting instead of automatic gameplay fallback. Do not activate the older candidate unchanged.
