...
export function IndependentHub({roomId}:{roomId?:string}){
 ...
 useEffect(()=>{
  if(!manifest||![12,13,14].includes(manifest.rulesVersion??4)||snapshot?.phase!==1||!arena.current||!bound?.epoch)return;
 ...
 useEffect(()=>{
  ...
  const restore=async()=>{
   ...
   try{
    ...
    if(manifest.rulesVersion===13||manifest.rulesVersion===14){
    ...
    play=new LabLane(fresh=>instance.feed.read(id,fresh),session,s.grant.player,receive,e=>{setControlled(false);setSync(recovery.failure(e));retryAt=Date.now()+recoveryDelay(e);},e=>setSync(recovery.failure(e)),{readMs:500,tickMs:300},{receipt:(result,name,args)=>instance.feed.receipt(id,result,name,receiptArgs(args),s.grant.player),sending:value=>pilot.sending(value)});
