...
export function SocialHub(p:Props) {
  ...
  useEffect(()=>{if(!p.account)return;let stop=false;void appApi(`/profiles/${p.account}`).then(d=>{if(!stop){setHandle(d.handle||"");setAvatar(d.avatar||0);}}).catch(()=>{});return()=>{stop=true;};},[p.account]);
  ...
  <div className="social-layout" hidden={!p.visible}>
    ...
    <div><details className="profile-editor"><summary>Your public profile</summary><section className="side-card"><p className="eyebrow">OPTIONAL PUBLIC PROFILE</p><h2>Make a name.</h2><p>Without a profile, only your address and onchain stats are public.</p><label>Unique nickname<input maxLength={20} value={handle} onChange={e=>setHandle(e.target.value.toLowerCase())} placeholder="3–20 characters, starting with a letter"/></label><div className="avatar-grid">{Array.from({length:12},(_,i)=><button key={i} aria-label={`Avatar ${i+1}`} aria-pressed={avatar===i} onClick={()=>setAvatar(i)}><Avatar index={i}/></button>)}</div><button disabled={!p.ready || busy || !p.account || !/^[a-z][a-z0-9_]{2,19}$/.test(handle)} onClick={()=>void run(async()=>{await auth();await appApi("/profiles","PUT",{handle,avatar});setMessage("Public profile saved.");await inspect(p.account);})}>Save public profile</button></section>
    <button disabled={!p.ready || busy || !p.account} onClick={()=>void run(async()=>{await auth();await appApi("/profiles","DELETE");setProfile(null);setHandle("");setMessage("Public profile removed. Onchain statistics remain public.");})}>Remove my public profile</button></details><Notebook disabled={!p.ready} account={p.account} authenticate={auth} identity={p.identity} matchRef={p.matchRef} atUs={p.atUs} applyPreferences={p.applyPreferences}/></div>
  </div></>;
}
