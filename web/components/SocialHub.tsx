...
export function SocialHub(p:Props) {
  ...
  useEffect(()=>{if(!p.ready || !p.account || p.config?.version!==3)return;let stopped=false;const unlock=()=>void p.authenticate().then(()=>{if(!stopped)setAuthenticated(true);}).catch(()=>{});unlock();const timer=setInterval(unlock,5000);return()=>{stopped=true;clearInterval(timer);};},[p.account,p.config?.version,p.ready]);
  ...
  async function action(c:any,action:string){await auth();const result=await appApi(`/challenges/${c.id}/${action}`,"POST",{});if(action==="accept"){setLinked(null);await refresh();entered.current.add(result.room_id);p.enter(result);}else await refresh();}
  async function create(){await auth();const c=await appApi("/challenges","POST",{recipient:target.trim()||null,mode,ranked});setLink(`${location.origin}/?challenge=${c.id}`);await refresh();setMessage("Challenge sent. Your opponent receives an Accept / Decline notification.");}
  ...
  <div className="social-layout" hidden={!p.visible}>
    <section className="side-card"><p className="eyebrow">PLAYER NETWORK</p><h2>Pick your rival.</h2><p>Friendly by default. Every match starts with two signed agreements.</p>
      ...
      <label>Opponent address<input value={target} onChange={e=>setTarget(e.target.value)} placeholder="Leave blank for a first-come open link"/></label>
