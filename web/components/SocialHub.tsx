...
type Props={mode:number;account:string;config:Config|null;visible:boolean;target:string;authenticate:()=>Promise<void>;identity:()=>Identity|null;open:()=>void;enter:(c:any)=>void;matchRef?:string;atUs?:string;applyPreferences:(settings:{preferredMode:number;sound:boolean})=>void};
export function SocialHub(p:Props) {
  ...
  useEffect(()=>{if(!p.account || p.config?.version!==3)return;let stopped=false;const unlock=()=>void p.authenticate().then(()=>{if(!stopped)setAuthenticated(true);}).catch(()=>{});unlock();const timer=setInterval(unlock,5000);return()=>{stopped=true;clearInterval(timer);};},[p.account,p.config?.version]);
  ...
  <div className="social-layout" hidden={!p.visible}>
    <section className="side-card"><p className="eyebrow">PLAYER NETWORK</p><h2>Pick your rival.</h2><p>Friendly by default. Every match starts with two signed agreements.</p>
      ...
      <button className="primary" disabled={busy || !p.account || !!target&&!/^0x[\da-fA-F]{40}$/.test(target)} onClick={()=>void run(create)}>Send challenge ↗</button>
