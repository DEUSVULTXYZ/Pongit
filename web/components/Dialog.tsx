...
export function CabinetTools({modal,open,onClose,children}:{modal:boolean;open:boolean;onClose:()=>void;children:ReactNode}){
  if(modal)return open?<Dialog label="Cabinet tools" onClose={onClose} className="cabinet-tools-dialog"><button className="modal-close" aria-label="Close cabinet tools" onClick={onClose}>×</button>{children}</Dialog>:null;
