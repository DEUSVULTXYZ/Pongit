...
export function IndependentHistory({player,matchId,rulesVersion=4}:{player:Address;matchId?:bigint;rulesVersion?:4|12}){
 ...
 async function load(id:string){
  ...
  try{for(;;){const r=await independentApi(`replay/${id}?after=${after}`);
   ...
   loaded.push(...r.frames.map(restoreEngineFrame));if(r.frames.length<1000)break;after=String(loaded.at(-1)!.revision);
