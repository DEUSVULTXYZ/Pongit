...
export function AgentReplay({reference}:{reference:AgentMatchRef}){
 const [frames,setFrames]=useState<EngineState[]>([]),[index,setIndex]=useState(0),[playing,setPlaying]=useState(false),[message,setMessage]=useState('Loading replay');
 useEffect(()=>{let done=false;setFrames([]);setIndex(0);setPlaying(false);
  void fetch(`${API}/agents/replay?${new URLSearchParams({id:reference.id,app:reference.app,epoch:reference.epoch})}`,{signal:AbortSignal.timeout(15000)})
   .then(async r=>{if(!r.ok)throw Error('Replay unavailable');return r.json();}).then(r=>{
    if(done)return;if(r.availability!=='available'){setMessage(r.availability==='pruned'?'Replay retired. The result remains available.':'No complete replay is available.');return;}
    const values=r.frames.map(restoreEngineFrame);setFrames(values);setMessage('REPLAY · Recorded contract snapshots');
 ...
 const s=frames[index];return <section><p role="status">{message}</p>{s&&<>
  ...
  <div className="rooms-canvas"><Court state={s.state} chaos={s.chaos} clock={s.clock} observedAt={Date.now()} direction={0} side={-1} replay
