...
/** Candidate HUD. Only a rules-6 transport with verified effects may supply it.
 ...
 return <div className={styles.hud} data-effects={effectsEnabled?'on':'off'} aria-label="Chaos effects">
  {visible.map(effect=>{
   ...
   return <div key={`${effect.slot}:${effect.startsAt}:${effect.id}`} className={styles.card} data-announcing={announcing||undefined}
    style={{'--chaos-color':definition.color,'--chaos-remaining':`${Math.max(0,Math.min(100,remaining/Math.max(1,duration)*100))}%`} as CSSProperties}>
