...
export function Arena({ initialTab = "Play" }: { initialTab?: string }) {
  ...
  return (
    <main className={`neon-rush cabinet-ui ${home?"at-home":""} ${match?.status===2 && side>=0 && ["Play","Live"].includes(tab)?"in-game":""}`}>
      ...
      {(tab==="Ladder") && <div className="mode-switch"><div className="control-segments" role="group" aria-label="Game mode"><button disabled={queued || busy || canControl} aria-pressed={mode===0} onClick={()=>setMode(0)}>01 / Classic</button><button disabled={queued || busy || canControl || tournamentId!=="0"} aria-pressed={mode===1} onClick={()=>setMode(1)}>02 / Chaos</button></div><p>{mode===1?"Crowd pressure shrinks the favourite's paddle. Changes apply between rallies.":"Pure Pong. Separate ranked ladder. First to seven."}</p></div>}
      ...
      {tab==="Archive" && account && <section className="match-list personal-replays"><p className="eyebrow">YOUR LAST THREE</p><h2>One more look.</h2>{recentError&&<p role="status">{recentError}</p>}{!recent.length&&!recentError&&<p>No completed games yet.</p>}{recent.map(m=><article key={m.id} className="recent-game"><span>{m.mode===1?"Chaos":"Classic"} · {m.ranked?"Ranked":"Friendly"}<small>{short(m.playerA)} vs {short(m.playerB)}</small></span><strong>{m.scoreA} : {m.scoreB}</strong><button disabled={m.replayAvailability!=="available"} onClick={()=>{if(m.deployment.startsWith("10143:"))window.open(`/rooms?history=${encodeURIComponent(m.id)}`,"_blank","noopener,noreferrer");else if(m.deployment===deploymentId(config!))void act(()=>loadReplay(m.rawId));else setLegacyReplay({deployment:m.deployment,id:m.rawId});}}>{m.replayAvailability==="available"?"Watch replay":m.replayAvailability==="pruned"?"Replay retired":"Replay indexing"}</button></article>)}</section>}
