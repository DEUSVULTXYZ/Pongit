...
export default async function TournamentsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <AgentTournaments enabled={process.env.PONG_AGENT_POOL_HOME==='true'&&process.env.PONG_AGENT_TOURNAMENTS_HOME==='true'} preview={process.env.PONG_AGENT_POOL_PREVIEW==='true'} initialId={typeof p.id==='string'&&/^[1-9]\d{0,18}$/.test(p.id)?p.id:undefined}/>;
}
