...
function ContractTable(){return <div className="docs-contracts">{docs.contracts.map(d=><section key={d.version}><h3>V{d.version} {d.version===4?"· Active":"· Archive"}</h3><p>Chain {d.chainId} · Index start block {d.startBlock}</p><div className="docs-table-wrap" tabIndex={0} role="region" aria-label={`V${d.version} contract addresses`}><table><thead><tr><th>Contract</th><th>Address</th></tr></thead><tbody>{d.addresses.map(a=><tr key={a.name}><td>{a.name}</td><td><a href={`https://testnet.monadscan.com/address/${a.address}`} target="_blank" rel="noopener noreferrer"><code>{a.address}</code></a></td></tr>)}</tbody></table></div></section>)}</div>;}
