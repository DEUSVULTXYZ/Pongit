import {openCabinet,openWallet,closeWallet,activity} from "./cabinet";
...
test("V2 Mera notebook, public profile, targeted friendly Chaos, handicap and arcade result",async({browser})=>{
  ...
  try{
    ...
    await a.getByLabel("Opponent address").fill(addresses[1]);await a.locator("summary").filter({hasText:"Match options"}).click();await a.getByLabel("Challenge mode",{exact:true}).selectOption("1");await expect(a.getByLabel("Competition")).toHaveValue("false");await a.getByRole("button",{name:"Send challenge"}).click();await a.getByText("Shareable links",{exact:true}).click();await expect(a.getByLabel("Share or open an invitation")).toHaveValue(/challenge=0x/);
    ...
    await openWallet(s);await s.getByRole("button",{name:"Get test credits"}).click();await expect(s.locator(".account-feedback")).toContainText("credited",{timeout:30000});await closeWallet(s);await s.getByRole("button",{name:/^Live/}).click();await s.locator(`[data-match-id="${match.id}"]`).click();await expect(s.locator(".chaos-warning")).toBeVisible();
    await s.getByLabel("Shares (1 winning share = 1 MON)").fill("0.006");
    for(let i=0;i<8;i++){await expect(s.getByRole("button",{name:"Back 01"})).toBeEnabled({timeout:15000});await s.getByRole("button",{name:"Back 01"}).click();await expect(s.getByRole("dialog",{name:"Review your bet"})).toBeVisible({timeout:20000});await s.getByRole("button",{name:"Confirm with passkey"}).click();await expect.poll(async()=> (await s.locator(".status-line").innerText()).includes("Bet confirmed") || await s.getByRole("button",{name:"Dismiss error"}).count()>0,{timeout:15000}).toBe(true);if((await s.locator(".status-line").innerText()).includes("Bet confirmed"))break;if(await s.getByRole("button",{name:"Close bet review"}).isVisible())await s.getByRole("button",{name:"Close bet review"}).click();await s.getByRole("button",{name:"Dismiss error"}).click();await s.waitForTimeout(600);}
    ...
    await openWallet(s);await s.getByText("Withdraw test MON",{exact:true}).click();await s.getByLabel("Recipient",{exact:true}).fill(addresses[2]);await s.getByLabel("Amount",{exact:true}).fill("0.001");await s.getByRole("button",{name:"Sign withdrawal"}).click();await expect(s.locator(".account-feedback")).toContainText("Withdrawal confirmed",{timeout:20000});await closeWallet(s);
    expect(BigInt((await (await s.request.get(api+`/player/${addresses[2]}`)).json()).balance)).toBe(beforeClaim+(config.version===4?-1000000000000000n:5000000000000000n));
    ...
    await a.getByRole("button",{name:"Connect passkey",exact:true}).click();await expect(a.locator(".status-line")).toContainText("Arcade session ready",{timeout:30000});await a.getByRole("button",{name:"Rivals",exact:true}).click();await a.getByRole("button",{name:"Unlock notebook"}).click();await expect(a.locator(".notebook")).toContainText("encrypted-rival-only");
    await activity(a,"Archive");await a.locator(`[data-match-id="${match.id}"]`).click();await expect(a.getByRole("button",{name:"Note this moment"})).toBeVisible({timeout:30000});
