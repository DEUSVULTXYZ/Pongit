...
test("Mera PRF: cancellation, financial signature, two players, spectator, recovery and replay", async ({browser}) => {
  ...
  try {
    ...
    const matchId = (await (await s.request.get(base + "/api/matches")).json()).matches.find((m:any)=>m.status===2 && [m.playerA,m.playerB].some((p:string)=>p.toLowerCase()===addresses[0].toLowerCase())).id;
    ...
    await expect.poll(async()=>{
      const d=await (await a.request.get(base+"/api/matches/"+matchId)).json();
    ...
    const beforeTouch=await (await b.request.get(base+"/api/matches/"+matchId)).json();
    ...
    await expect.poll(async()=>{
      const current=await (await b.request.get(base+"/api/matches/"+matchId)).json();
