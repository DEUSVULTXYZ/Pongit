...
test("Home usernames persist, remain unique and identify rivals, arena and ladder",async({browser})=>{
 ...
 try {
  for(let i=0;i<2;i++){
   ...
   await p.getByRole("button",{name:"Close profile"}).click();await expect(p.locator(".home-player-tag")).toContainText(i===0?handle:other);
   await p.reload();await expect(p.locator(".home-player-tag")).toContainText(i===0?handle:other,{timeout:15000});
   await expect(p.locator(".home-player-tag img")).toHaveAttribute("src",`/avatars/roster-v1/${i===0?"vector":"pulse"}.webp`);
  }
  const [a,b]=pages;
  ...
  await b.locator(".profile-results").getByRole("button").filter({hasText:handle}).click();await expect(b.getByLabel("Opponent address",{exact:true})).toHaveValue(addresses[0].toLowerCase());await expect(b.locator(".public-profile img")).toHaveAttribute("src","/avatars/roster-v1/vector.webp");
