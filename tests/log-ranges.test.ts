import test from 'node:test';import assert from 'node:assert/strict';import {chunkedLogs,historyGate} from '../relayer/src/log-ranges';
...
test('large historical batches have a global concurrency bound and release failed slots',async()=>{
 const gate=historyGate(4);let active=0,max=0;const settled=await Promise.allSettled(Array.from({length:30},(_,i)=>gate(async()=>{max=Math.max(max,++active);await new Promise(r=>setTimeout(r,2));active--;if(i===3)throw Error('failed batch');return i;})));assert.equal(max,4);assert.equal(active,0);assert.equal(settled.filter(x=>x.status==='fulfilled').length,29);assert.equal(await gate(async()=>42),42);
});
