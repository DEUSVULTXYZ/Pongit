...
for(const viewport of [{width:360,height:640},{width:390,height:844},{width:768,height:1024},{width:1440,height:1000}])test(`Avatar roster is readable and keyboard accessible at ${viewport.width}px`,async({browser})=>{
 ...
 try {
  ...
  expect(await form.locator(".portrait-grid").evaluate(e=>getComputedStyle(e).gridTemplateColumns.split(" ").length)).toBe(viewport.width<=480?3:4);
  await radios.nth(0).click();await expect(radios.nth(0)).toHaveAttribute("aria-checked","true");
  await page.keyboard.press("ArrowRight");await expect(radios.nth(1)).toBeFocused();await expect(form.locator(".character-identity")).toHaveText("Ghost · Night rider");
  await page.keyboard.press("ArrowDown");await expect(radios.nth(viewport.width<=480?4:5)).toBeFocused();
  await page.keyboard.press("End");await expect(radios.nth(11)).toBeFocused();await expect(form.locator(".character-preview img")).toHaveAttribute("src","/avatars/roster-v1/viper.webp");
