...
for(const viewport of [{width:360,height:640},{width:390,height:844},{width:768,height:1024},{width:1440,height:900},{width:844,height:390}])test(`Cabinet navigation, focus and static room ${viewport.width}x${viewport.height}`,async({browser})=>{
 ...
 await page.getByRole("button",{name:"Arcade settings",exact:true}).click();await expect(page.getByRole("dialog",{name:"Arcade settings"})).toContainText("Karl Casey @ White Bat Audio");const top=await page.evaluate(()=>document.body.style.top);await page.mouse.wheel(0,300);expect(await page.evaluate(()=>document.body.style.top)).toBe(top);await page.keyboard.press("Escape");await expect(page.getByRole("dialog")).toHaveCount(0);
