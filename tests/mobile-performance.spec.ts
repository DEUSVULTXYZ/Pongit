...
test("mobile replay performance under CPU and network emulation",async({browser})=>{
 ...
 try{
  await p.goto(base);await p.getByRole("button",{name:"Enter muted"}).click();await p.getByRole("button",{name:"Archive",exact:true}).click();await p.locator('[data-match-id="1"]').click();await expect(p.getByRole("button",{name:/^Play replay/})).toBeVisible({timeout:30000});await p.getByRole("button",{name:/^Play replay/}).click();
