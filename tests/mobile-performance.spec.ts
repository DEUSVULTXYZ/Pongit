...
test("mobile replay performance under CPU and network emulation",async({browser})=>{
 ...
 try{
  await p.goto(base);await p.getByRole("button",{name:"Enter muted"}).click();await activity(p,"Archive");await p.locator(".match-row").filter({hasText:"FINAL"}).first().click();await expect(p.getByRole("button",{name:/^Play replay/})).toBeVisible({timeout:30000});await p.getByRole("button",{name:/^Play replay/}).click();
