...
const agentApi=process.env.PONG_AGENT_API??`http://pongit-agent-service-${stamp}:4100`;
assert.match(agentApi,/^http:\/\/pongit-agent-service-20[0-9]{6}(-[2-9])?:4100$/,'Point at a private laboratory service');
const agents=await fetch(agentApi+'/live').then(r=>r.json());
