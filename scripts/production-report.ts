...
try{
 const jobs=(await db.query("SELECT id,payload,status,raw_tx,receipt,tx_hash,nonce,created_at,signed_at,submitted_at,confirmed_at FROM relay_jobs WHERE payload->>'deployment'=$1 AND receipt IS NOT NULL ORDER BY nonce",[version])).rows;
 if(!jobs.length)throw new Error("No receipts recorded for this deployment yet");
 const rooms=(await db.query("SELECT match_id,job_id FROM rooms WHERE deployment=$1 AND match_id IS NOT NULL",[version])).rows;
 ...
 const rows=jobs.map(j=>{const transaction=parseTransaction(j.raw_tx),r=j.receipt;const payload=j.payload,args=payload.args;const matchId=map.get(j.id) || (payload.contract==="game"? (typeof args[0]==="object"?args[0].matchId:payload.functionName==="playerAction"?args[1]:args[0]):payload.contract==="market"?(typeof args[0]==="object"?args[0].matchId:args[0]):undefined);
  return {hash:j.tx_hash,nonce:j.nonce,contract:payload.contract,transition:payload.functionName,matchId,status:j.status,gasUsed:r.gasUsed,gasLimit:transaction.gas,effectiveGasPrice:r.effectiveGasPrice,feeWei:transaction.gas!*BigInt(r.effectiveGasPrice),valueWei:r.status==="success"?BigInt(payload.value || 0):0n,createdAt:j.created_at.toISOString(),confirmedAt:j.confirmed_at.toISOString(),queueMs:+j.signed_at-+j.created_at,broadcastMs:+j.submitted_at-+j.signed_at,confirmationMs:+j.confirmed_at-+j.submitted_at,totalMs:+j.confirmed_at-+j.created_at};});
 const gameIds=[...new Set(rows.filter(r=>r.contract==="game" && r.matchId).map(r=>String(r.matchId)))];
 const matchCosts=gameIds.map(id=>{const game=rows.filter(r=>r.contract==="game"&&String(r.matchId)===id);return {matchRef:`${version}:${id}`,gameTransitions:game.length,gameGasWei:game.reduce((a,b)=>a+b.feeWei,0n),definition:"Game creation, reveals, inputs, resolutions, rating and result actions recorded by this sponsor; includes reverted game calls. Excludes direct callers, bets, withdrawals, faucet credits and market liquidity."};});
