...
async function write(module:'tournaments'|'vault',fn:string,args:unknown[],value=0n){const request={address:d[module],abi:abi[module] as any,functionName:fn,args,value,account:wallet.account};const estimate=await client.estimateContractGas(request);const hash=await wallet.writeContract({...request,gas:estimate*130n/100n+10_000n});const receipt=await client.waitForTransactionReceipt({hash});assert.equal(receipt.status,'success',hash);/* Low-balance operator: let Monad's reserve-balance window clear before the next value spend. */while(await client.getBlockNumber({cacheTime:0})<receipt.blockNumber+4n)await new Promise(r=>setTimeout(r,350));return hash;}
...
const createHash=await write('tournaments','create',[now+40n,2,fee],parseEther('0.002'));
...
while((await client.getBlock()).timestamp<=now+40n)await new Promise(r=>setTimeout(r,700));
