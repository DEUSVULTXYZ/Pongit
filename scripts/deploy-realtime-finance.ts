...
const record=JSON.parse(await readFile('/secrets/realtime-20260913.json','utf8'));assert.equal(record.state,'answered');
...
try{
 ...
 const job=(await t.db.query('SELECT hash FROM il_lifecycle_jobs WHERE id=$1',['realtime-20260913:deploy-roomsrealtimesettlement'])).rows[0];
 const block=(await t.base.getTransactionReceipt({hash:job.hash})).blockNumber;
 const finance={financeId:'realtime-v1',settlement:'early-published-testnet',betting:'realtime',app,adapter,market,vault,pressureSigner:old.pressureSigner,startBlock:String(block),chainId:10143};
 const oldGame=JSON.parse(await readFile('deployments/interlude-rooms.json','utf8'));
 const game={...oldGame,app,node:record.node,rulesVersion:5,releaseReady:false,releaseStatus:'qualifying-realtime-betting',previousRooms:oldGame.app};
