...
import { marketV4Abi, tournamentsV4Abi } from "../shared/abis-v4";
...
if(legacy.chainId!==chainId || legacy.version!==3)throw new Error("Expected V3 manifest on the same chain");
...
const lmsr = legacy.lmsr;
if (!await publicClient.getCode({address:lmsr})) throw new Error("Previous LMSR bytecode missing");
const market = await deploy("MarketV4", [admin, treasury, game, lmsr, vault]);
const tournaments = await deploy("TournamentsV4", [admin, treasury, game, vault]);
...
const path =
  ...
  `deployments/${chainId === 31337 ? "local-v4" : "testnet-v4"}.json`;
await mkdir("deployments", { recursive: true });
await writeFile(
  path,
  JSON.stringify(
    {
      version:4, legacy, arcade,
