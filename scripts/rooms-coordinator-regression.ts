...
try{
 ...
 const oldTab=await action(pa,'offers/accept',{id:offer.id});
 assert.equal(oldTab.status,400);assert.match(oldTab.error,/Refresh PONGIT/);
 assert.equal((await action(pa,'offers/accept',{id:offer.id,countdown:true})).status,400,'A missing intro cannot pass the new preflight');
 ...
 assert.equal((await action(pb,'offers/accept',{id:offer.id})).status,400,'Old clients cannot bypass a running intro either');
 ...
 slowSnapshot=true;const accepting=action(players[0],"offers/accept",{id:offer.id,countdown:true});await pause(100);
