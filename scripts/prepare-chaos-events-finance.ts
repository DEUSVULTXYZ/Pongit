...
try{
 assert.equal(await t.base.readContract({address:record.app,abi:roomsEventsAbi,functionName:'RULES_VERSION'}),8n);
 ...
 const game={app:record.app,node:record.node,hub:'0x3Ef8327F69e09cf721772F345e2A887eA22cD595',coordinator:coordinator.address,pressureSigner:pressure.address,rulesVersion:8,compactControls:true,capacity:2,chainId:10143,previousRooms:zeroAddress,releaseReady:false};
 const finance={financeId:'events-rules8',rulesVersion:8,settlement:'early-published-testnet',betting:'realtime',app:record.app,adapter,market,vault,pressureSigner:pressure.address,startBlock:String(block),chainId:10143};
