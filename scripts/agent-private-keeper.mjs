...
while(!stopped){
 try{
  ...
  if(next.renewalQualified){
   ...
   if(!exists){docker('run','-d','--name',name,'--network',network,'--user=1000:1000','--cpus=.25','--memory=256m','--cap-drop=ALL','--security-opt=no-new-privileges','--env-file',root+'/private/service.env','-e','PONG_AGENT_SERVICE=0','-e','PONG_AGENT_SOAK=dedicated-24h','-e',`PONG_AGENT_API=http://pongit-agent-service-${stamp}:4100`,'-v',root+'/release:/work:ro','-v',secret+'/manifest.json:/secrets/manifest.json:ro','-v',metadata+':/metadata:ro','-e','PONG_AGENT_LIFECYCLE_RECORD=/metadata/lifecycle.json','-v',root+'/diagnostics:/diagnostics','-w','/work',image,'node','/app/node_modules/tsx/dist/cli.mjs','scripts/agent-soak.ts');}
