...
const file=`/secrets/compact-live-20260913-${run}.json`;try{await readFile(file);throw Error('Reconcile the previous live fixture first');}catch(e){if((e as any).code!=='ENOENT')throw e;}
...
const t=await chainTools('compact-live-20260913-'+run),config=await loadRoomsFinance(),base=t.base;
 ...
 for(const mode of [0,1]){
  ...
  while(Date.now()-start<150000){
   ...
   for(let j=0;j<2;j++){
    ...
    if(direction!==p.dir){const latest:any=await snapshot(id);if(latest[2]!==2n)break;
     const at=Date.now(),args=[id,direction,latest[j===0?9:10]+1n,latest[7]+150n],result=await p.session.send('input',args);await feed.receipt(id,result,'input',args,p.owner.address);p.dir=direction;p.changes++;(report.inputMs??=[]).push(Date.now()-at);
...
finally{unwatch.forEach(stop=>stop());stream.stop();report.rpc=takeRpcSamples();await save();await writeFile(`artifacts/realtime/compact-live-${run}.json`,json(report));console.log(json({passed:report.passed,error:report.error,matches:report.matches.map((m:any)=>({mode:m.mode,score:m.score,frames:m.frames.length,changes:m.changes})),payments:report.payments}));await t.close();if(!report.passed)process.exitCode=1;}
