...
export async function chainTools(prefix:string,fetchFn?:typeof fetch){
 ...
 async function deploy(name:string,args:readonly unknown[]=[],instance=name):Promise<Address>{
  ...
  const maxRuntime=['SeriesAgentArena','AgentSeriesPool','IndependentEventsArena','IndependentEventsLobby','ReadyIndependentEventsArena','ReadyIndependentEventsLobby'].includes(name)?32768:24576;
