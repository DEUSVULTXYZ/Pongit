...
try{
 ...
 if(!record.startedEpoch){record.startedEpoch=String(d.epoch);await save();}
 if(force&&record.renewalQualified){await event('qualification-cycle-complete',{epoch:m.epoch});}
  ...
  if(active){await event(now>=d.expiresAt?'intervention-required':'draining',{active,...(now>=d.expiresAt?{reason:'Delegation expired with unfinished games. Reconcile published state before closing; do not discard uncertain commands.'}:{})});}
   ...
   else{const tx=await t.write(`close-${d.epoch}`,m.app,abi,'closeEngine');await event('closed',{epoch:String(d.epoch),tx:tx.transactionHash});}
  }
 }else if(d.status===2){
  ...
  if(!valid){
   ...
   else{await db.query("UPDATE agent_arcade.control SET admissions=true,reason='',updated_at=now() WHERE app=$1",[m.app]);record.startedEpoch=m.epoch;record.renewalReady=true;await save();await event('online',{epoch:m.epoch});}
  }
