...
const artifact=JSON.parse(await readFile('contracts/out/PongRoomsRealtime.sol/PongRoomsRealtimeRelease.json','utf8'));
...
if(record){assert.equal(record.codeHash,codeHash);assert(record.app,'Creation remains uncertain; inspect the existing attempt');console.log(JSON.stringify(record));}
 ...
 record={purpose:'realtime-chaos-rules5',chainId:10143,codeHash,state:'sending',at:new Date().toISOString()};
 ...
 try{
  const response=await fetch('https://control.interludelayer.xyz/apps',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:'PongRoomsRealtimeRelease',bytecode:artifact.bytecode.object,abi:artifact.abi.filter((x:any)=>x.type==='constructor'||x.type==='function'&&x.name==='delegateAll')}),signal:AbortSignal.timeout(120000)});
