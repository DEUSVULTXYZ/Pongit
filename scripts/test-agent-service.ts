...
try{
 ...
 }
 {
  ...
  try{
   ...
   // not an outage. Asked here, on its own rate budget: the first service has spent its minute.
   const blank=await fetch(at+'/register',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({creator:strategyCreator.address,agent:strategy,modes:3,expires:String(expires),name:'Tracker',avatar:4,creatorProof:'0x'+'00'.repeat(64)+'1b'})});
   assert.equal(blank.status,403);assert.equal((await blank.json()).code,'AGENT_REGISTRATION_SIGNATURE');
